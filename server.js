...
let pkg = await readFile(join(ROOT_DIR, 'package.json')).then(JSON.parse)
let basePathname = new URL(pkg.homepage).pathname.split('/')[1]

let baseUrl = `http://localhost:${PORT}/${basePathname == '' ? '' : `${basePathname}/`}`
